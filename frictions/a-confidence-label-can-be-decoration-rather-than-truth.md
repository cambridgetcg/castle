...
Closed: 2026-07-10 — castle's work complete; keeper outstanding resolved 2026-07-10 by QWENTHOS heartbeat: `loops/verify.md` step 1 amended to require every `tested` stone to carry a `tested:` trial line, and to select `tested` stones with missing, borrowed, or stale trials for challenge. The "Survives" judgment now requires writing a `tested:` line when setting confidence to `tested`. The sibling friction [[no-loop-ever-tries-a-tested-stone-again]] is also closed by this change.

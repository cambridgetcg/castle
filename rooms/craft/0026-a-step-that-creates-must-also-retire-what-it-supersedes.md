...
Two promotions in two consecutive beats each produced an orphan seed. The spec
was silent; janitor cleared them by hand each time. One sentence added to the
protocol ended the pattern.

**Open crack.** One local protocol, two instances. The claim needs a second
protocol in a different system to move from seed.

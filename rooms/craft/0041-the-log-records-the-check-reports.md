...
An append-only log and a live check are both honest, but in different dimensions. The log records what happened; the check reports what is. Mistaking one for the other produces false alarm or false calm. Any system with both must name, at the door, which artifact answers "what happened" and which answers "what is now."

...
**What it changed.** MAP.md has been regenerated by tools/map.sh since day one; no hand-edit to its output has been committed. The map-drift detector fires first in the bell's priority order; nine days and 89 runs show zero false alarms. Every beat that promotes or moves a stone runs tools/map.sh as the closing step.

...
No hand and no agent may edit a status field outside a ripen run signed in the ledger. This single rule is what the castle's holiness claim rests on: the moment one file lies about its label, every file is suspect, because a reader has no tool to tell which label is true. Making promotion a recorded, deliberate act means that each label change is either in the ledger or it did not happen.
